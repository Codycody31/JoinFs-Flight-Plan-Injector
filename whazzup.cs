...
namespace FSHS_Desktop_ATC
{
    class whazzup   // revision 11
    {
        ...
        public void UpdateWithFlightPlans()
        {
            ...
            try{con.Open();}
            catch{logger.error("Failed to connect to database", "whazzup", "UpdateWithFlightPlans");}
            MySqlDataReader reader = cmd.ExecuteReader();
            //Ini Configure
            var MyIni = new IniFile();
            var path = MyIni.Read("whazzup", "Data");
            //open file
            try { new StreamReader(File.OpenRead(path)); }
            ...
            {
                logger.error("Failed to WriteAllText for whazzup_tfl.txt", "whazzup", "UpdateWithFlightPlans");
            }
            ...
            {
                logger.error("Failed to update whazzup_tfl with aircraft", "whazzup", "UpdateWithFlightPlans");
            }
            CLIENTS_tfl = null;
        ...
        public void DeleteClients(string whazzupPath = null)
        {
            ...
            logger.info("whazzup_tfl.txt deleted", "whazzup", "DeleteClients");
        }
        public void WriteClients()
        {
            ...
            string text = null;
            try { File.ReadAllText("whazzup_TFL.txt"); }
